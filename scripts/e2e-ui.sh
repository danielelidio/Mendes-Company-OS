#!/usr/bin/env bash
# Roda os testes E2E no modo UI do Playwright (interativo, visível).
...
usage() {
  cat <<EOF
${BOLD}Uso:${RESET} ./scripts/e2e-ui.sh [--help]

Abre os testes E2E no ${BOLD}modo UI${RESET} do Playwright (interativo/visível).
Garante os emuladores Firebase antes.
